...
export default function TheSidebar(props: {
  ...
  return (
    <div
      className={`lg:w-[300px] md:w-[250px] w-[calc(100vw-16px)] md:mr-0 md:static p-3 lg:p-5 border-2 border-gray-200 bg-white flex-col ${
        show ? "flex absolute" : "md:flex hidden absolute"
      }`}
    >
      ...
      <div className="flex flex-wrap justify-center md:mt-8">
        <img src={logo_short} alt="Logo" className="w-70 h-auto mb-2" />
      </div>
      <div className="flex mt-12 pb-10 h-full flex-col [&>a]:flex [&>a]:select-none [&>a]:items-center [&>a]:gap-4 [&>a]:px-4 lg:[&>a]:px-8 [&>a:hover]:bg-gray-100 [&>a]:duration-200 [&>a]:py-1 [&>a]:mx-2 [&>a]:rounded-md text-lg">
        ...
        <Link
          to={token ? `/book/user/${userId}` : "/login"}
